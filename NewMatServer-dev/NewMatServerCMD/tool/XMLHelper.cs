...
public class XMLHelper
{
    ...
    public static byte[] DataSetToXML(DataSet ds)
    {
        ...
        try
        {
            ...
            {
                XmlNode mainData = xmlDoc.CreateElement(dt.TableName);
                root.AppendChild(mainData);
                int dtColumnCount = dt.Columns.Count;
                for (int j = 0; j < dtColumnCount; j++)
                {
                    XmlElement xe = xmlDoc.CreateElement(dt.Columns[j].ColumnName);
                    xe.InnerText = dt.Rows[i][j].ToString();
                    mainData.AppendChild(xe);
                }
            }
